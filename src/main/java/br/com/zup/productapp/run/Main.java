...
import br.com.zup.productapp.embedded.EmbeddedMongo;
...
/**
 * 
 * @author Luan Roque. Classe de execução do sistema.
 * 
 *
 */
public class Main {
	...
	private static synchronized DB mongo() throws Exception {
		...
		MongoClient mongoClient = new MongoClient(
				new ServerAddress(Constants.DataSource.HOST, Constants.DataSource.PORT), mongoClientOptions);
		DB db = mongoClient.getDB(Constants.DataSource.DB_NAME);
		return db;
	}

	private static synchronized void mongoEmbedded() {
